...
export default function BienesPage() {
  ...
  const cargarBienes = async () => {
    try {
      ...
      const res = await fetch(`/api/caracteristicas-inmueble/completitud`, { headers: { 'Content-Type': 'application/json' }, cache: 'no-store' });
      const completitudData = await res.json();
      const registros = completitudData?.data?.registros || [];
      
      // Listado de Bienes: mostrar solo registros con completado === 100
      const registrosCompletos = registros.filter((r: { completado?: number }) => (r?.completado ?? 0) === 100);
      ...
      const bienesCompletos = (registrosCompletos as ApiRegistro[]).map((registro) => ({
      ...
      console.log('[Bienes] Datos cargados para tabla:', bienesCompletos);
      setBienes(bienesCompletos);
      setTotal(bienesCompletos.length);
    } catch (error) {
